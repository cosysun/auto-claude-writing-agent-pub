...
[![Version: 1.3](https://img.shields.io/badge/Version-1.3-orange)]()
...
## 🎉 v1.3 重要更新（2025-11-23）

### ✨ 新增"首要原则"（最高优先级）

**问题背景**：AI无法跨会话记忆，之前经常跳过关键流程步骤，导致质量下降。

**解决方案**：在规则文档最前面新增"首要原则"章节，明确标注9个⭐关键步骤：

**必须执行的9个关键步骤**：
1. ✅ **知识库** - 搜索后必须保存到 `_knowledge_base/`
2. ✅ **选题讨论** ⭐⭐⭐ - 绝不直接写文章，必须先提供3-4个选题方案
3. ✅ **协作文档** - 需要测试时必须创建
4. ✅ **个人素材库** ⭐⭐⭐ - 必须搜索 `_personal_materials/`（降AI味核心）
5. ✅ **创意排水** ⭐ - 重要文章必做
6. ✅ **版本管理** - draft-v1 → v2 → v3 → final（严格版本控制）
7. ✅ **三遍审校** ⭐⭐⭐ - 内容→风格→细节，每遍创建新版本
8. ✅ **标题拟定** ⭐⭐⭐ - 提供3-5个方案，等待用户选择
9. ✅ **配图Prompt** ⭐ - 提供5-8张配图的Prompt（3个版本）
...
> **v1.3 重要更新**：新增"首要原则"，明确标注9个⭐关键步骤必须执行，强化版本管理（v1→v2→v3→final），防止AI跳过关键流程。

```
Step 1: 理解需求 & 保存Brief ⭐
  ↓ 明确目标、受众、核心信息
  
Step 2: 搜索调研 & 知识库 ⭐ [v1.3强化：搜索后必须保存]
  ↓ 收集最新资料、验证数据、保存到知识库
  
Step 3: 选题讨论 ⭐⭐⭐（必做）[v1.3强化：绝不跳过]
  ↓ 3-4个选题方案，让你选择（绝不直接写文章）
  
Step 4: 协作文档（如需测试）[v1.3强化：需要测试时必须创建]
  ↓ 明确测试任务和配图需求
  
Step 5: 风格学习 & 个人素材库 ⭐⭐⭐ [v1.3强化：必须搜索]
  ↓ 分析历史文章，搜索个人素材（降AI味核心）
  
Step 6: 准备测试数据
  ↓ 可选：等待真实测试数据
  
Step 6.5: 创意排水 ⭐（推荐）[v1.3强化：重要文章必做]
  ↓ 排空"废水"（套路想法），找到"清水"（独特创意）
  
Step 7: 创作初稿（版本管理开始）[v1.3强化：严格版本控制]
  ↓ 按你的风格写作，保存为 draft-v1.md
  
Step 8: 三遍审校 ⭐⭐⭐（核心）[v1.3强化：每遍创建新版本]
  ↓ 第一遍：内容（逻辑、事实、结构）→ v2
  ↓ 第二遍：风格（降AI味、口语化）→ v3
  ↓ 第三遍：细节（标点、排版、节奏）→ final
  
Step 9: 标题拟定 ⭐⭐⭐ [v1.3强化：必须提供方案]
  ↓ 提供3-5个标题方案（爆款版 + 自然版）
  
Step 10: 配图准备 ⭐ [v1.3强化：3个版本Prompt]
  ↓ 提供5-8张配图Prompt（每张3个版本）
...
│   ├── CLAUDE.md               # 微信公众号规则文档（1085行，v1.3）
│   ├── README.md               # 项目说明
│   │
│   ├── _briefs/                # 需求文档
...
- **更新频率**：周更
- **适合**：技术分享、深度思考、教程类
- **规则文档**：`mp-wechat/CLAUDE.md`（1085行，v1.3）
- **v1.3核心更新**：
  - ✅ 首要原则章节（最高优先级，防止跳过关键步骤）
  - ✅ 9个⭐关键步骤检查清单（知识库、选题讨论、协作文档、个人素材库、创意排水、版本管理、三遍审校、标题拟定、配图Prompt）
  - ✅ 严格版本管理要求（draft-v1 → v2 → v3 → final，每次审校创建新版本）
  - ✅ 灵活性说明（只有用户明确要求时才可跳过步骤）
...
AI会引导你走完10步流程，你只需要：
- 在选题环节选择你喜欢的角度（Step 3，⭐⭐⭐必做，v1.3强化：绝不跳过）
- 在关键决策时确认或调整
- 在审校环节提出修改意见（Step 8，⭐⭐⭐核心，v1.3强化：每遍创建新版本）
- 在标题拟定时选择喜欢的版本（Step 9，⭐⭐⭐，v1.3强化：必须提供方案）
- 最后检查和发布

> **v1.3提示**：AI现在会严格执行所有⭐关键步骤，不会随意跳过。如果你想跳过某步骤，需要明确告知。
...
### Q7: v1.3 新增的"首要原则"是什么？为什么需要？

**问题背景**：
- ❌ AI无法跨会话记忆，每次对话都是"新的一天"
- ❌ 之前经常跳过关键流程步骤（如选题讨论、版本管理、标题拟定）
- ❌ 导致文章质量不稳定、返工率高

**v1.3 解决方案**：
- ✅ 在规则文档最前面新增"首要原则"章节（最高优先级）
- ✅ 明确标注 9 个⭐关键步骤，必须执行
- ✅ 强化版本管理（v1 → v2 → v3 → final，每次审校创建新版本）
- ✅ 明确灵活性规则（只有用户明确要求时才可跳过）
...
### v1.2（2025-11-08）✅ - 标题拟定优化
- [x] 新增：完整的标题设计流程
  - 第一轮：爆款标题方案（基于7要素）
  - 第二轮：自然风格标题（无套路版）
  - 第三轮：组合优化（可选）
- [x] 标题要素分析和推荐机制
- [x] 主体问题检查（作者主导 vs 工具主导）

### v1.3（2025-11-23）✅ - 流程强化
- [x] 新增："首要原则"章节（最高优先级）
- [x] 明确：9个⭐关键步骤检查清单
- [x] 强化：严格版本管理要求（v1 → v2 → v3 → final）
- [x] 强化：选题讨论、个人素材库、三遍审校的必做要求
- [x] 防止：AI跳过关键流程步骤
